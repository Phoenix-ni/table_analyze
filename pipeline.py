...
def fill_excel_with_ocr_content(empty_excel_path, output_excel_path, grid_to_yolo, merge_info, merge_ranges, img_path, ocr_engine):
    ...
    for (i, j), yolo_box in grid_to_yolo.items():
        ...
        cv2.putText(debug_img, f"{i},{j}", (x1 + 5, y1 + 20),
...
def main():
    ...
    empty_excel_path = "empty_table_skeleton.xlsx"
    final_excel_path = "final_table_with_content.xlsx"
    ...
    print(f"表格尺寸: {rows} 行 × {cols} 列")
    print(f"合并单元格数量: {len(merge_ranges)}")

    print("步骤2: YOLO检测单元格...")
    yolo_boxes = run_yolo(model_path, img_path, conf=0.25)
    print(f"检测到 {len(yolo_boxes)} 个单元格")

    print("步骤3: 将YOLO框映射到网格...")
    grid_to_yolo = map_yolo_to_grid(yolo_boxes, grid_x, grid_y)
    print(f"成功映射 {len(grid_to_yolo)} 个单元格到网格")
    ...
    print("🎉 处理完成！")
    print(f" - 空表格框架: {empty_excel_path}")
    print(f" - 最终表格: {final_excel_path}")
    print(f" - 调试图像: debug_yolo_to_grid.jpg")
